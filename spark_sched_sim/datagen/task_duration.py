...
class TaskDurationGen:
    ...
    def __init__(self, task_duration_data, np_random, warmup_delay=1000):
    ...
    def sample(
        ...
        executor_key = self._sample_executor_key(num_local_executors, executor_interval_map)

        if executor.task is None or executor.job_id != task.job_id:
            # the executor never runs a task in this job fresh executor incurs a warmup delay
            try:
        ...
        if executor.task.stage_id == task.stage_id:
            # executor was working on this node the task duration should be retrieved from rest wave
            try:
    ...
    def _sample_executor_key(self, num_local_executors, executor_interval_map):
        left_exec, right_exec = executor_interval_map[num_local_executors]
