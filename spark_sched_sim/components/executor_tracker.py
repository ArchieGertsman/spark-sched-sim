...
class ExecutorTracker:
    ...
    def reset(self):
        ...
        # reside at this pool
        self._pools = {None: set(), COMMON_POOL_KEY: set(range(self.num_executors))}

        # pool key A ->
        #   (pool key B ->
        #       number of commitments from
        #       pool A to pool B)
        self._commitments = {None: {}, COMMON_POOL_KEY: {}}

        # pool key -> total number of outgoing commitments
        # from this pool
        self._num_commitments_from = {None: 0, COMMON_POOL_KEY: 0}

        # stage pool key -> total number of commitments to stage
        self._num_commitments_to_stage = {COMMON_POOL_KEY: 0}
        ...
        # of its stages
        self._total_executor_count = {None: 0}
