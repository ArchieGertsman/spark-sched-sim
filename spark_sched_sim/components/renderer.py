...
class Renderer:
    ...
    def render_frame(
        ...
        # draw text
        text_surfaces = self._make_text_surfaces(
            wall_time, avg_job_duration, num_active_jobs, num_jobs_completed
        )
    ...
    def _draw_worker_histories(self, canvas, worker_histories, wall_time):
    ...
    def _draw_job_completion_markers(self, canvas, job_completion_times, wall_time):
        for t in job_completion_times:
            x = self.window_width * (t / wall_time)

            pygame.draw.rect(
                canvas, (255, 0, 0), pygame.Rect((x, 0), (1, self.window_height))
            )

    def _make_text_surfaces(
        ...
        surfs = [
            ...
            self.font.render(
                f"Avg job duration: {avg_job_duration}s", False, (255,) * 3
            ),
            self.font.render(f"Num active jobs: {num_active_jobs}", False, (255,) * 3),
            self.font.render(
                f"Num jobs completed: {num_jobs_completed}", False, (255,) * 3
            ),
        ]
