...
class ActorNetwork(nn.Module):
    def __init__(
        ...
        self.encoder = EncoderNetwork(
            num_node_features, dim_emb, num_encoder_layers)

        emb_dims = {
            'node': dim_emb * num_encoder_layers,
            'dag': dim_emb,
            'glob': dim_emb
        }
        hid_dims = [64, 64, 1]
...
class EncoderNetwork(nn.Module):
    def __init__(self, num_node_features, dim_embed, num_layers):
        super().__init__()
        self.node_encoder = NodeEncoder(num_node_features, dim_embed, num_layers)
        self.dag_encoder = DagEncoder(dim_embed * num_layers, dim_embed)
...
class NodeEncoder(nn.Module):
    def __init__(self, num_node_features, dim_embed, num_layers, reverse_flow=True):
        super().__init__()
        self.reverse_flow = reverse_flow
        self.j, self.i = (1, 0) if reverse_flow else (0, 1)

        self.lin_init = nn.Linear(num_node_features, dim_embed)

        self.attn_convs = nn.ModuleList(
            [AttnConv(dim_embed, reverse_flow) for _ in range(num_layers)])
        # flow = 'target_to_source' if reverse_flow else 'source_to_target'
        # self.attn_convs = nn.ModuleList([
        #     gnn.GATv2Conv(
        #         dim_embed, dim_embed, add_self_loops=False, flow=flow) 
        #     for _ in range(num_layers)
        # ])
        
        self.gru_cells = nn.ModuleList(
            [nn.GRUCell(dim_embed, dim_embed) for _ in range(num_layers)])
...
class GlobalEncoder(nn.Module):
    def __init__(self, dim_emb):
        super().__init__()
        self.lin = nn.Linear(dim_emb, dim_emb)
