...
from ..graph_utils import obs_to_pyg, ObsBatch
...
class PolicyNetwork(nn.Module):
    ...
    def _compute_dag_scores(
        ...
        executor_actions = torch.arange(self.num_executors, device=dag_embeddings.device)
        executor_actions_rpt = executor_actions.repeat(num_dags).unsqueeze(1)

        num_total_actions = executor_actions_rpt.shape[0]
        ...
        dag_scores = self.mlp_dag_score(dag_inputs) \
                         .squeeze(-1) \
                         .view(num_dags, self.num_executors)
